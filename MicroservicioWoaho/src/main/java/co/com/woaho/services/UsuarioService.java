...
import co.com.woaho.enumeraciones.EnumCorreos;
...
import co.com.woaho.utilidades.AdministradorCorreos;
...
public class UsuarioService implements IUsuarioService{
	...
	@Autowired
	private IMensajeCorreoDao mensajeCorreoDao;
		...
		try {
			...
			if (usuario == null) {
				...
				if(strRespuesta[0].equalsIgnoreCase(EnumGeneral.RESPUESTA_NEGATIVA.getValor())) {
					...
					MensajeCorreo mensajeCorreo = mensajeCorreoDao.obtenerMensajeIdioma(EnumCorreos.CORREO_CODIGO_ACCESO.getCodigo(), request.getIdioma());

					new AdministradorCorreos().enviarCorreo("Access Code", ProcesarCadenas.getInstance().obtenerMensajeFormat(mensajeCorreo.getMensajeCorreoMensaje(),usuario.getNombreCompleto(),strRespuesta[1]), usuario.getStrCorreo(), parametroDao);
	...
	public void generarCodigoIngreso(Usuario pUsuario,String pIdioma) throws Exception {
		try {
			...
			if(strRespuesta[0].equalsIgnoreCase(EnumGeneral.RESPUESTA_NEGATIVA.getValor())) {
				...
				MensajeCorreo mensajeCorreo = mensajeCorreoDao.obtenerMensajeIdioma(EnumCorreos.CORREO_CODIGO_ACCESO.getCodigo(), pIdioma);

				new AdministradorCorreos().enviarCorreo("Access Code", ProcesarCadenas.getInstance().obtenerMensajeFormat(mensajeCorreo.getMensajeCorreoMensaje(),pUsuario.getNombreCompleto(),strRespuesta[1]), pUsuario.getStrCorreo(), parametroDao);
			}	
		}catch(Exception e) {
